...
#define IRAM_SAFE_SIZE IRAM_SAFE_END - IRAM_SAFE_START
    ...
    void clear_iram(void)
    {
        ...
        {
            ams::exosphere::CopyToIram(IRAM_SAFE_START + ofs, g_work_page, sizeof(g_work_page));
        }
    }
    ...
    void DebugLog(const char* format, ...)
    {
        ...
        if (g_curr_log_offset >= 4)
        {
            ...
            ams::exosphere::CopyFromIram(prev_4, IRAM_SAFE_START + g_curr_log_offset - 4, 4);
            ...
            if (prev_4_size < 4)
            {
                ...
                ams::exosphere::CopyToIram(IRAM_SAFE_START + g_curr_log_offset - 4, prev_4, 4);
        ...
        if (g_curr_log_offset + msg_len_aligned > IRAM_SAFE_SIZE)
        {
            ...
            g_page_num++;
            clear_iram();
            ams::exosphere::CopyToIram(IRAM_SAFE_END - sizeof(u32), &g_page_num, sizeof(u32));
        }
        ...
        if (iram_out_start + msg_len_aligned > iram_next_page)
        {
            ...
            ams::exosphere::CopyToIram(iram_out_start, g_work_page, first_page_size);
            ams::exosphere::CopyToIram(iram_next_page, g_work_page + first_page_size, msg_len_aligned - first_page_size);
        }
        else
        {
            ams::exosphere::CopyToIram(iram_out_start, g_work_page, msg_len_aligned);
        }
